%%-------------------------------------------------------------------------------------
function [ParaSetting,Population]=Transformationnew(Population,ParaSetting,AR,radius1)
    ...
    rr = cauchyrnd(0,1,[ParaSetting.SchoolNum,1]);%Generate 1 Cauchy random number 
	for k = 1 : ParaSetting.SchoolNum
        ...
        if rand > 1-sqrt(k/ParaSetting.SchoolNum)
		    Population(ParaSetting.SchoolSize*(k-1)+1:ParaSetting.SchoolSize*k,1:end-1) = particalCoordinate ;
        else
            Population(ParaSetting.SchoolSize*(k-1)+1:ParaSetting.SchoolSize*k,1:end-1)=rr(k)*particalCoordinate+particalCoordinate;
        end
	end
